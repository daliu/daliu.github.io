...
\begin{abstract}
\noindent Single-subject psychiatric polygenic risk score (PRS) computation from consumer-chip genotyping (e.g., 23andMe v5) is widely practiced but methodologically fraught. We document an empirical observation with practical implications: the dominance of chromosome 6 major histocompatibility complex (MHC) tag single-nucleotide polymorphisms (SNPs) in chip-PRS for schizophrenia is \emph{specific to European-cohort genome-wide association studies (GWAS)} and \emph{absent from East-Asian-cohort GWAS}. The Lam et al.~2019 EAS-cohort GWAS itself notes that several MHC variants reach genome-wide significance in European cohorts but not in East Asians due to allele-frequency differences (e.g., rs13194504 minor allele frequency $<1\%$ in EAS vs $9\%$ in EUR; the C4-BS allele rare in Han Chinese); we document the practical consequence for single-subject chip-PRS calculations, which to our knowledge has not been demonstrated explicitly. In a single Han Chinese individual genotyped on 23andMe v5, cross-referencing against the PGC3 wave~3 EUR-cohort schizophrenia GWAS (Trubetskoy et al.~2022) yields approximately 1{,}491 typed genome-wide-significant (GWS) SNPs of which $\sim$66\% fall in the chr 6 extended MHC region (25--34 Mb) --- driving an uncalibrated centered PRS (defined here as $\sum_i \beta_i (d_i - 1)$, where $d_i \in \{0,1,2\}$ is effect-allele dosage) that collapses by approximately two orders of magnitude when MHC SNPs are excluded. The same subject cross-referenced against the Lam et al.~2019 EAS-cohort GWAS yields 54 typed GWS SNPs, \emph{zero} of which are in the MHC region. The dominant per-locus signal in the EAS-cohort analysis lives at the chromosome 10q24 BORCS7/AS3MT/NT5C2 cluster (8 of 13 typed SNPs homozygous for the risk-direction allele), the chromosome 12q24 cluster (510 GWS SNPs in a single 1-Mb window), and several chromosome 2 and 3 loci. The practical implication is that for East Asian subjects, ancestry-matched chip-PRS is more biologically interpretable than for European subjects --- the LD-inflation problem that dominates EUR-cohort chip-PRS is essentially absent. We complement the analysis with a per-variant pharmacogenomic profile and a coverage audit identifying four CPIC-actionable East Asian variants (\emph{ADH1B} *2, \emph{CYP2D6} *10, \emph{HLA-B*15:02}, \emph{HLA-B*58:01}) not typed by 23andMe v5, limiting its utility as a stand-in for a clinical pharmacogenomic panel in this population.
\end{abstract}
...
The 66\% MHC share of EUR-cohort GWS SNPs in the typed subset is consistent with previously published observations on chip-PRS and HLA \cite{trubetskoy2022,sekar2016}: the MHC is one of the strongest reproducible psychiatric loci, has dense long-range LD, and is intensively typed for HLA imputation purposes on chips designed for European-ancestry users. The 0\% MHC share in the EAS-cohort GWS SNP set is consistent with the Lam et al.~2019 authors' own reported observation \cite{lam2019} that several MHC variants reach genome-wide significance in EUR but not in EAS due to allele-frequency differences --- specifically rs13194504 with EAS minor allele frequency $<1\%$ vs $9\%$ in EUR, and the C4-BS allele being uncommon in Han Chinese and Korean populations. The smaller EAS case sample size (22{,}778 vs PGC3's 76{,}755) further reduces statistical power at MHC tag SNPs. We did not adjudicate between these two contributions in the present case analysis; the methodological consequence (chip-PRS not MHC-dominated for EAS subjects) is robust regardless of which dominates.
...
The chromosome 6 MHC dominance that characterizes single-subject psychiatric chip-PRS in European-cohort analyses is essentially absent when an ancestry-matched East Asian-cohort GWAS is used as the reference, consistent with the allele-frequency differences explicitly reported by the Lam 2019 authors \cite{lam2019}. For the East Asian subject in this case study, the dominant per-locus signal in the EAS-cohort schizophrenia analysis lives at chromosome 10q24 (BORCS7/AS3MT/NT5C2 cluster), chromosome 12q24, chromosome 2p16 (VRK2/ZFP36L2), and several chromosome 3 loci. The non-MHC chip-PRS landscape becomes more interpretable when the LD-inflation source is removed by the cohort change. Combined with a CPIC-aligned per-variant pharmacogenomic profile and a coverage audit identifying ADH1B *2, CYP2D6 *10, HLA-B*15:02, and HLA-B*58:01 as East Asian-clinically-actionable variants not typed on 23andMe v5, this analysis provides a single-subject methodological characterization of consumer-chip psychiatric genomics for East Asian users.
