...
Runs daily (launchd: com.daveliu.keep-credentials-warm). Three jobs:
...
3. Commit + push a changed calendar.json so the live dashboard updates.

Neither credential can be re-authed non-interactively. When a refresh fails with
a "needs re-auth" signature we fire a macOS notification (and record status) so
the one-time manual fix can be run:
...
not syncing. If auth is healthy but biometrics are missing, the watch hasn't
uploaded to Garmin Connect — sync the watch; no software here can substitute.
